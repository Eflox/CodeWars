 ...
 * each pair represent number of people get into bus (The first item) and number
 ...
 * bus station (after the last array). Even though it is the last bus stop, the bus is
 * not empty and some people are still in the bus, and they are probably sleeping there :D
 * 
...
public class Kata
{
    public static int Number(List<int[]> peopleListInOut)
    {
        ...
        foreach (var stops in peopleListInOut)
        {
        pplIn += stops[0];
        pplOut += stops[1];
        }
